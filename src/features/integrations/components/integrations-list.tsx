...
export const IntegrationsList = ({ query, setQuery, integrations, selected, setSelected }: any) => {
    return (
        <div className="flex flex-col md:h-[670px] h-full">
            <Card className="border-0 bg-card/70 backdrop-blur flex flex-col h-full">
                <CardContent className="p-4 flex flex-col h-full flex-1 min-h-0">
                    ...
                    {/* Search */}
                    <SearchBar placeholder="Search Integrations..." value={query} onChange={setQuery} />
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2 mb-1">
                                        <h3 className="text-sm font-medium truncate">{it.name}</h3>
                                        <div
                                            className={clsx(
                                                "h-2 w-2 rounded-full flex-shrink-0",
                                                it.connected ? "bg-green-500" : "bg-muted-foreground/30"
                                            )}
                                        />
                                    </div>

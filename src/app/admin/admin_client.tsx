...
const AdminClient = () => {
    ...
    const handleApprovalAction = async (
        ...
        try {
            await axios.post(
                "/api/admin",
                { projectId, approved: newApprovedStatus },
                config
            );
            ...
            toast.success(
                `Project ${newApprovedStatus ? "Approved" : "Rejected"}!`,
                { theme: "dark" }
            );
        } catch (err) {
            console.error(
                `Error ${
                    newApprovedStatus ? "approving" : "rejecting"
                } project:`,
                err
            );
                ...
                {isLoading ? (
                    ...
                    >
                        ...
                        <p className="text-slate-400">
                            There are no projects pending approval right now.
                        </p>
                    </motion.div>
                ) : filteredProjects.length === 0 && searchQuery ? (
                    <motion.div
                            ...
                            {filteredProjects.map((project, index) => (
                                ...
                                >
                                    ...
                                    <div className="p-5 flex flex-col flex-grow">
                                        <h3 className="text-lg font-semibold text-sky-300 mb-1 line-clamp-2">
                                            {project.title}
                                        </h3>
                                        ...
                                        {project.github && (
                                            ...
                                            >
                                                <FaGithub className="mr-1.5 group-hover:text-sky-300" />{" "}
                                                View on GitHub{" "}
                                                <FaExternalLinkAlt className="ml-1.5 opacity-70 group-hover:opacity-100" />
                                            </a>
                                        )}

...
export const NavItem = ({
    ...
    const baseClasses =
        "relative font-medium transition-colors duration-200 ease-in-out group";
    const mobileClasses = `block w-full text-left px-4 py-3.5 rounded-lg text-base ${baseClasses} ${
        isActive
            ? "bg-sky-500/20 text-sky-300"
            : "text-slate-200 hover:bg-slate-700/60 hover:text-sky-300"
    }`;
...
export const NavButton = ({
    ...
    const base =
        "font-semibold rounded-lg transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 transform";
    const mobile = `block w-full text-left px-4 py-3.5 text-base ${base}`;
    const desktop = `px-5 py-2.5 text-sm ${base}`;

    const primary =
        "bg-gradient-to-r from-sky-500 to-blue-600 text-white hover:from-sky-600 hover:to-blue-700 focus:ring-sky-500 shadow-md hover:shadow-lg";
    const secondary =
